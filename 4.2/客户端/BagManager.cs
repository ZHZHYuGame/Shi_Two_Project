...
public class BagManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        MessageCenter.Ins.AddListener(NetID.S_To_C_Add_Bag_Data_Message,GetBagData);
        MessageCenter.Ins.AddListener(NetID.User_To_Bag_Set_Money_Message,SetMoney);
    }

    public void AddBtnClick()
    {
        int count = BagCellCount - 30 / 10;
        consume.OnSetText($"是否消耗{count}个闪电扩展背包？",$"{count}/{UserManager.Ins.lightning}","Goods/闪电");
        consume.noClick = () => { gameObject.SetActive(false); };
        consume.yesClick = () => {
            if (UserManager.Ins.lightning >= count)
            {
                BagCellCount += 5;
                var index = bagList.Count;
                for (int i = 0; i < 5; i++)
                {
    ...
    private void SetMoney(object obj)
    {
        moneyText.text = UserManager.Ins.money.ToString();
    }
    ...
    private void UpdateEqCell(List<Item> list)
    {
        ...
        {
            eqDic[(EqPositionType)i].SetValue(list[i]);
