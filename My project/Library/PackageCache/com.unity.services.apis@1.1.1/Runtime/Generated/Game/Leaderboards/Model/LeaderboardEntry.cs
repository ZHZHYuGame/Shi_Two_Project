
//-----------------------------------------------------------------------------
// <auto-generated>
//     Changes to this file will be lost if the code is regenerated.
// </auto-generated>
//-----------------------------------------------------------------------------
using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using UnityEngine.Scripting;
using Unity.Services.Apis;

namespace Unity.Services.Apis.Leaderboards
{
    /// <summary>
    /// LeaderboardEntry
    /// </summary>
    [DataContract(Name = "LeaderboardEntry")]
    [Preserve]
    public partial class LeaderboardEntry
    {
        /// <summary>
        /// Gets or Sets PlayerId
        /// </summary>
        [DataMember(Name = "playerId", IsRequired = true, EmitDefaultValue = true)]
        [Preserve]
        public string PlayerId { get; set; }

        /// <summary>
        /// Gets or Sets PlayerName
        /// </summary>
        [DataMember(Name = "playerName", IsRequired = true, EmitDefaultValue = true)]
        [Preserve]
        public string PlayerName { get; set; }

        /// <summary>
        /// Gets or Sets Rank
        /// </summary>
        [DataMember(Name = "rank", IsRequired = true, EmitDefaultValue = true)]
        [Preserve]
        public int Rank { get; set; }

        /// <summary>
        /// Gets or Sets Score
        /// </summary>
        [DataMember(Name = "score", IsRequired = true, EmitDefaultValue = true)]
        [Preserve]
        public double Score { get; set; }

        /// <summary>
        /// Gets or Sets Tier
        /// </summary>
        [DataMember(Name = "tier", EmitDefaultValue = false)]
        [Preserve]
        public string Tier { get; set; }

        /// <summary>
        /// Gets or Sets Metadata
        /// </summary>
        [DataMember(Name = "metadata", EmitDefaultValue = false)]
        [Preserve]
        public string Metadata { get; set; }

        /// <summary>
        /// Initializes a new instance of the <see cref="LeaderboardEntry" /> class.
        /// </summary>
        /// <param name="playerId">playerId (required).</param>
        /// <param name="playerName">playerName (required).</param>
        /// <param name="rank">rank (required).</param>
        /// <param name="score">score (required).</param>
        /// <param name="tier">tier.</param>
        /// <param name="metadata">metadata.</param>
        [Preserve]
        public LeaderboardEntry(string playerId = default(string), string playerName = default(string), int rank = default(int), double score = default(double), string tier = default(string), string metadata = default(string))
        {
            if (playerId == null)
            {
                throw new ArgumentNullException("playerId is a required property for LeaderboardEntry and cannot be null");
            }
            this.PlayerId = playerId;
            if (playerName == null)
            {
                throw new ArgumentNullException("playerName is a required property for LeaderboardEntry and cannot be null");
            }
            this.PlayerName = playerName;
            this.Rank = rank;
            this.Score = score;
            this.Tier = tier;
            this.Metadata = metadata;
        }
    }

}
